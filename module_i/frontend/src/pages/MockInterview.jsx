...
const MockInterview = () => {
  ...
  const startInterview = async () => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:8080/api/interview/generate-questions', formData);
  ...
  const getFeedback = async (finalQaPairs) => {
    ...
    setPhase('FEEDBACK');
    try {
      const res = await axios.post('http://127.0.0.1:8080/api/interview/evaluate', { qaPairs: finalQaPairs });
